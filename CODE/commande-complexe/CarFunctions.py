...
def UpdateCar(R,Speed,position,target,orientation,direction,Rimpact,reverse):
    ...
    if Speed < Parameters.Max_reverse_speed :
        Speed = Parameters.Max_reverse_speed
    ...
    vmaxant=1.1*sqrt(-2/3*parameters.Min_acceleration*abs(Rimpact)) #Maximum speed to give to dodge a wall        
    Max_speed_allowed=min(vmaxr,parameters.Car_maxspeed,vmaxant) # Max velocity allowed
    Max_speed_allowed=min(vmaxr,parameters.Car_maxspeed)
    ...
    NewSpeed=min(Parameters.Car_maxspeed,Speed+a*parameters.Lidar_delta,Max_speed_allowed)
